...
public class NotificationHostView extends FrameLayout {
    ...
    private class NotificationView extends FrameLayout {
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        if (v.getWidth() > 0) {
                            ...
                            if (canBeDismissed() && (mShownNotifications == 0 || (shown
                                    && mShownNotifications == 1))) {
                                NotificationHostView.this.setBackgroundColor(Color.argb(MAX_ALPHA -
                                        (int) (Math.abs(xr) / v.getWidth() * MAX_ALPHA), 0, 0, 0));
                            }
                            if (swipeGesture  || Math.abs(event.getX() - initialX) >
                                    CLICK_THRESHOLD) {
                                swipeGesture = true;
        ...
        public float getVelocity() {
            if (pointerDown) {
                velocityTracker.computeCurrentVelocity(1); // 1 = pixel per millisecond
            }
    ...
    public NotificationHostView(Context context, AttributeSet attributes) {
        ...
        mNotificationMinHeight = mContext.getResources().getDimensionPixelSize(
                R.dimen.notification_min_height);
        mNotificationMinRowHeight = mContext.getResources().getDimensionPixelSize(
    ...
    public void onFinishInflate() {
        if (NotificationViewManager.config != null) {
            ...
            int maxHeight = Math.round(
                    mDisplayHeight - mDisplayHeight * NotificationViewManager.config.offsetTop);
        ...
        if (NotificationViewManager.NotificationListener != null) {
            try {
                ...
                for (StatusBarNotification sbn : sbns) {
                    if ((dismissedSbn = mDismissedNotifications.get(
                            describeNotification(sbn))) == null
    ...
    public boolean addNotification(StatusBarNotification sbn, boolean showNotification,
            boolean forceBigContentView) {
    ...
    private void handleAddNotification(final boolean showNotification,
            boolean forceBigContentView) {
        ...
        if (reposted && oldView.bigContentView) {
            forceBigContentView = true;
        }
        boolean bigContentView = sbn.getNotification().bigContentView != null && ((reposted
                && oldView.bigContentView)
                || (NotificationViewManager.config.expandedView || sbn.getNotification().contentView
                == null));
        nv.bigContentView = bigContentView && forceBigContentView;
        RemoteViews rv = nv.bigContentView ? sbn.getNotification().bigContentView
                : sbn.getNotification().contentView;
        final View remoteView = rv.apply(mContext, null);
        remoteView.setLayoutParams(new LayoutParams(mDynamicWidth ? LayoutParams.WRAP_CONTENT
        ...
        if (reposted){
            // The notification already exists, so it was just changed. Remove the old view and add
            // the new one
            ...
            if (showNotification && !oldView.shown && showNotification && !oldView.pointerDown) {
    ...
    private void handleRemoveNotification(final boolean dismiss) {
        ...
        if (mNotifications.containsKey(describeNotification(sbn)) && sbn != null) {
            ...
            int duration =  getDurationFromDistance(v.getChildAt(0), v.shown ? -mDisplayWidth
                    : mDisplayWidth, 0);
    ...
    private void showNotification(NotificationView nv) {
        if (!NotificationViewManager.config.privacyMode) {
            ...
            boolean useRealVelocity = !(Math.copySign(1, nv.getVelocity()) == Math.copySign(1,
                    v.getX()));
            int duration = useRealVelocity ? getDurationFromDistance(v, targetX, 0, Math.abs(
                    nv.getVelocity())) : ANIMATION_MAX_DURATION;
    ...
    private void hideNotification(NotificationView nv) {
        ...
        int duration = getDurationFromDistance(v, targetX, (int)v.getY(), Math.abs(#
                nv.getVelocity()));
        if (mShownNotifications > 0 && nv.shown) {
            mShownNotifications--;
        }
        if (mShownNotifications == 0) {
            animateBackgroundColor(0);
    ...
    private void setButtonDrawable() {
        ...
        try {
            statusBar = IStatusBarService.Stub.asInterface(ServiceManager.getService(
    ...
    private void animateBackgroundColor(final int targetColor) {
        ...
        final ObjectAnimator colorFade = ObjectAnimator.ofObject(this, "backgroundColor",
                new ArgbEvaluator(), ((ColorDrawable) getBackground()).getColor(), targetColor);
    ...
    private void animateTranslation(final NotificationView v, final float targetX,
            final float targetY, final int duration) {
    ...
    public Notification getNotification(StatusBarNotification sbn) {
        if (containsNotification(sbn)) {
            return mNotifications.get(
                    describeNotification(sbn)).statusBarNotification.getNotification();
        } else {
            return null;
        }
    }
    ...
    private int getDurationFromDistance (View v, int targetX, int targetY, float ppms) {
        ...
        float y = v.getY();
        if (targetY == y) {
            distance = Math.abs(Math.round(x) - targetX);
        } else if (targetX == x) {
            distance = Math.abs(Math.round(y - targetY));
        } else {
            distance = (int) Math.abs(Math.round(Math.sqrt((x - targetX)*(x * targetX)+(y - targetY)*(y - targetY))));
        }
